...
for (const s of selectedSources) {
  ...
  try {
    ...
    out[s.id] ??= {};
    // Each line gets its OWN guard baked alongside it (not just the shared
    // record-level one below) — different lines of the same series can have
    // genuinely different plausible ranges (e.g. life expectancy vs healthy
    // life years), and without this a release-assurance check re-verifying
    // the baked file has no way to know a line's real range, only whichever
    // range the first-processed entry for this id happened to declare.
    if (s.line) {
      const line = { id: s.line, points };
      if (s.min != null && s.max != null) line.guard = { min: s.min, max: s.max };
      (out[s.id].lines ??= []).push(line);
    } else {
      out[s.id].points = points;
    }
    ...
    // Record-level guard: the range the series' PRIMARY value (top-level
    // `points`, e.g. the representative line used for tile summaries) was
    // validated against — surfaced in the UI as a visible QA signal
    // ("Quality check: latest value validated within X-Y"). First writer
    // wins so this only ever reflects the non-`line` entry when one exists.
...
const file =
  ...
  `export type RawSeries = { points?: RawPoint[]; lines?: { id: string; points: RawPoint[]; guard?: { min: number; max: number } }[]; asOf?: string; srcUrl?: string; guard?: { min: number; max: number }; srcHash?: string; srcBytesHash?: string };\n` +
